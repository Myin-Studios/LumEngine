...
public:
	...
	Shader(const std::string& vertexPath, const std::string& fragmentPath, const std::string& geometryPath = "") : vertPath(vertexPath.c_str()), fragPath(fragmentPath.c_str()), geomPath(geometryPath.c_str()), ID(0)
	{
	...
	Shader(const Shader& s) : vertPath(s.vertPath), fragPath(s.fragPath), ID(s.ID), geomPath(s.geomPath)
	{

	}
	Shader(Shader&& s) noexcept : vertPath(std::move(s.vertPath)), fragPath(std::move(s.fragPath)), ID(std::move(s.ID)), geomPath(std::move(s.geomPath))
	{
		std::string vertexCode;
		std::string fragmentCode;
		std::ifstream vShaderFile;
