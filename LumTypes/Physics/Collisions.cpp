...
std::vector<LumEngine::Physics::AABB> LumEngine::Physics::RayCast::boundingVolumes = {};

LumEngine::Physics::RayCastResult LumEngine::Physics::RayCast::Cast(Vec3Core& origin, Vec3Core& direction)
{
...
void LumEngine::Physics::RayCast::ScreenToRay(int mouseX, int mouseY, int width, int height, Vec3Core& origin, Vec3Core& direction)
{
    ...
    std::cout << "Near world: " << nearWorld.ToString() << std::endl;
    std::cout << "Far world: " << farWorld.ToString() << std::endl;

    origin = Vec3Core(nearWorld);
    direction = (Vec3Core(farWorld - nearWorld)).Normalize();

    std::cout << "Ray origin: " << origin.ToString() << std::endl;
    std::cout << "Ray direction: " << direction.ToString() << std::endl;
}

bool LumEngine::Physics::RayCast::IntersectRayBounds(const Vec3Core& origin, const Vec3Core& direction, const AABB& bounds, float& distance, Vec3Core& hitPoint)
{
...
void LumEngine::Physics::RayCast::RegisterBoundingVolume(std::vector<Vertex> verts)
{
    ...
    boundingVolumes.emplace_back(verts);
